...
Comparison operators (||, &&) and ternary formats are reassuringly identical across both languages, as are booleans (e.g. >=) and arithmetic operators other than division (`x/y`).  The default division operation is floating point in JavaScript, so Ruby: `puts 5/2 => 2`, JS: `console.log(5/2) => 2.5`.  Also, Javascript treats Infinity as a result in division, so Ruby: `puts 5/0 => zero division error, JS: `console.log(5/0) => Infinity`.
...
`deepEqual` solution was not good, need to try again later
